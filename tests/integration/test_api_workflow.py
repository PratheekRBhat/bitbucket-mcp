...
class TestEndToEndPRWorkflow:
    ...
    def test_complete_pr_lifecycle(self):
        ...
        # List open PRs
        prs = client.pull_requests.list(state="open")
        assert len(prs) == 2
        assert all(isinstance(pr, PullRequest) for pr in prs)

        # Get specific PR
        pr = client.pull_requests.get(pull_request_id=1)
        assert pr.id == 1
        ...
        # Merge PR
        merged_pr = client.pull_requests.merge_simple(pull_request_id=1)
    ...
    def test_decline_pr_workflow(self):
        ...
        # Decline PR
        declined_pr = client.pull_requests.decline(pull_request_id=3)
    ...
    def test_authentication_headers_present(self):
        ...
        client = BitbucketClient(workspace=workspace, repo_slug=repo)
        client.pull_requests.get(pull_request_id=1)
...
class TestErrorHandlingWorkflow:
    ...
    def test_handles_404_not_found(self):
        ...
        with pytest.raises(BaseClientError):
            client.pull_requests.get(pull_request_id=999)
